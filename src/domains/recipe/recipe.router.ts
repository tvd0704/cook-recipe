...
recipeRouter.post(
  ...
  upload.single("photo"),
  catchError(recipeController.addRecipe)
);

recipeRouter.delete(
  "/:recipeId",
  validateDto(recipeDto.deleteRecipe),
  authJwt,
  checkRole([UserRole.User]),
  catchError(recipeController.deleteRecipe)
);

recipeRouter.put(
  "/:recipeId",
  validateDto(recipeDto.updateRecipe),
  authJwt,
  checkRole([UserRole.User]),
  catchError(recipeController.updateRecipe)
);

recipeRouter.get(
  "/",
  validateDto(recipeDto.getAllRecipe),
  authJwt,
  checkRole([UserRole.User]),
  catchError(recipeController.getAllRecipe)
);
...
recipeRouter.get(
  ...
  authJwt,
  checkRole([UserRole.User]),
  catchError(recipeController.searchRipe)
);

recipeRouter.get(
  "/:recipeId",
  validateDto(recipeDto.getRecipe),
  authJwt,
  checkRole([UserRole.User]),
  catchError(recipeController.getRecipe)
);
