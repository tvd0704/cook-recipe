...
export const login = async ({email, password}:ILogin): Promise<string> => {
    const existingUser = await userRepository.findOne({
        where: { email },
        select: ["userId", "firstName", "lastName", "email", "phoneNumber", "status", "password", "role"]
    })
    if (!existingUser) throw new AppError("The email does not exist", "NotFound", 404);
...
export const findOne = async ({
  ...
  checkActive = false }: IFindOne): Promise<User | null> => {
  const existingUser = await userRepository.findOneBy(query);

  if (checkExistence.flag && !existingUser) {
    throw new AppError(checkExistence.errorMsg ?? "Account does not exist", "NotFound", 404);
  }

  if (checkActive && existingUser && existingUser.status !== UserStatus.Active) {
    throw new AppError("The account has not been activated", "BadRequest", 400);
  }
